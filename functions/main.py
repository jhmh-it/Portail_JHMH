...
def before_user_signed_in(
    ...
    try:
        ...
        if not is_email_allowed(user_email):
            message = f"Connexion bloquée pour: {user_email} - Domaine non autorisé"
            print(message)
            ...
            raise identity_fn.HttpsError(
                ...
                message="Accès restreint aux employés JHMH. Veuillez utiliser "
                        "votre adresse email professionnelle @jhmh.com"
            )
        ...
        error_msg = f"Erreur lors de la vérification d'authentification: {str(e)}"
        print(error_msg)
        raise identity_fn.HttpsError(
...
def test_domain_check(req: https_fn.Request) -> https_fn.Response:
    ...
    result = {
        ...
        "message": f"Email {'autorisé' if is_allowed else 'bloqué'} "
                   f"pour la plateforme JHMH"
    }
