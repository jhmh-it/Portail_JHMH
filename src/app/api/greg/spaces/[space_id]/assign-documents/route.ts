...
export async function POST(
  ...
  try {
    ...
    // Appel à l'API externe pour assigner les documents
    const apiUrl = `${apiBaseUrl}/api/greg/spaces/${fullSpaceId}/assign-documents`;
    ...
    if (!response.ok) {
      ...
      console.error(
        "[API] Erreur lors de l'assignation des documents:",
        errorText
      );
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Impossible d'assigner les documents",
          details: errorText,
        },
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Erreur lors de l'assignation des documents",
      },
      { status: 500 }
    );
  }
