...
/**
 * Fetches actifs from the API
 */
async function fetchActifs(filters: ActifsFilters): Promise<ActifsResponse> {
  ...
  if (!result.success) {
    throw new Error(result.error ?? 'Failed to fetch actifs');
  }
...
/**
 * Hook personnalisé pour gérer les actifs
 */
...
/**
 * Prefetch actifs data for performance optimization
 */
export function prefetchActifs(
...
/**
 * Invalidate actifs cache to force refetch
 */
export function invalidateActifs(queryClient: QueryClient): Promise<void> {
